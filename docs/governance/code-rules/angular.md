...
version: 1.1
current: true
description: Angular 19+ DO / NEVER + snippets. Standalone, Signals, OnPush, inject() not constructor DI.
---
...
- **NEVER** inject dependencies through the **constructor** (legacy constructor dependency injection). Use **`inject()`** in class fields instead. See [Dependency injection](#dependency-injection-inject-not-constructor).
- **NEVER** add an empty or logic-only **constructor** when the same behavior fits a **field initializer** (e.g. `afterNextRender`, `afterEveryRender`, `effect` registered at creation time).
...
- Use **`inject()`** for dependency injection in components, services, and functional APIs — not constructor parameters.
...
**Dependency injection** means Angular (or your test) **provides** services (`HttpClient`, routers, facades) to a class instead of the class calling `new Service()` itself.

### What is obsolete

- **Constructor dependency injection:** listing services as `constructor(private http: HttpClient)` parameters. Do not use for new code.

### What is allowed

- The **`constructor` keyword is not banned.** Avoid it for injection and for one-off setup when a field initializer is clearer.
- **`inject()` in class fields** — default for components and `@Injectable` services.
- **`inject()` inside functional guards, interceptors, resolvers** — same rule.
- **Field initializers in the injection context** for browser-only or post-render setup (`afterNextRender`, `effect`, etc.) instead of a constructor body.

### Browser-only data loads (SSR apps)

When a component must call HTTP or read app-relative assets **only in the browser** (SSR cannot resolve those URLs reliably):

- Prefer **`afterNextRender()` in a field initializer**, not `ngOnInit` on the server and not a constructor body.
- Keep fetch logic in a **service**; the component only schedules the call.
...
**Inject dependencies with `inject()` (required):**
...
// ❌ Bad — constructor dependency injection (legacy)
constructor(private http: HttpClient) {}
```

**Browser-only init with `afterNextRender` (no constructor):**

```typescript
protected readonly browse = inject(ManualsBrowseService);

private readonly scheduleLoad = afterNextRender(() => {
  void this.browse.loadManuals();
});
```

**Functional guard (same injection rule):**
